...
void printUsage(const char *arg0) {
  printf("usage: %s [-h] [--platform <name>] [--type <cpu,gpu,acc>]\n", arg0);
}

void parseArguments(const int argc, const char **argv,
                    const char **platform_name, const char **device_type) {
  for (int argi = 1; argi < argc; argi++) {
    ...
    } else if (0 == strcmp("--type", argv[argi])) {
      argi++;
      if (argi == argc) {
        printUsage(argv[0]);
        fprintf(stderr, "error: expected device type [cpu,gpu,acc]\n");
        exit(1);
      }
      *device_type = argv[argi];
    } else {
      printUsage(argv[0]);
...
cl_uint selectDevices(cl_platform_id selected_platform,
                      const char *device_type_arg,
  ...
  } else if (strcmp(device_type_arg, "gpu") == 0) {
    device_type = CL_DEVICE_TYPE_GPU;
  } else if (strcmp(device_type_arg, "cpu") == 0) {
    device_type = CL_DEVICE_TYPE_CPU;
  } else if (strcmp(device_type_arg, "acc") == 0) {
  ...
  IS_CL_SUCCESS(
      clGetDeviceIDs(selected_platform, device_type, 0, NULL, &num_devices));
  ...
  if (1 == num_devices) {
    fprintf(stderr,
            "Expected more than 1 devices, but only 1 device found, exiting\n");
    exit(1);
  }
  ...
  IS_CL_SUCCESS(clGetDeviceIDs(selected_platform, device_type, num_devices,
                               devices, NULL));
...
int main(const int argc, const char **argv) {
  ...
  parseArguments(argc, argv, &platform_name, &device_type);

  cl_platform_id selected_platform = selectPlatform(platform_name);
  num_devices = selectDevices(selected_platform, device_type, &devices);
  ...
  for (i = 0; i < num_devices; i++) {
    ...
    cl_context context = clCreateContext(
        NULL, /* num_devices */ 1, devices + i, NULL, NULL, &errcode);
    ...
    cl_command_queue queue =
        clCreateCommandQueue(context, *(devices + i), 0, &errcode);
    IS_CL_SUCCESS(errcode);
    ...
    IS_CL_SUCCESS(clEnqueueNDRangeKernel(
        queue, kernel, /* work_dim */ 1,
        /* global_work_offset */ NULL, &global_work_size, NULL, 0, NULL, NULL));
